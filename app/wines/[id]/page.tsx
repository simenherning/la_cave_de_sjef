...
export default async function WinePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: wine }, { data: notes }, { data: externalNotes }] = await Promise.all([
    ...
    supabase.from('external_notes').select('*').eq('wine_id', id).order('note_date', { ascending: false, nullsFirst: false }),
  ])
  ...
  return (
    <WineDetail
      wine={wine as Wine}
      notes={(notes ?? []) as TastingNote[]}
      externalNotes={(externalNotes ?? []) as ExternalNote[]}
    />
  )
}
